...
this configuration is stable across all workloads at low-to-moderate utilization (<20% peak block fullness) and provides the lowest block confirmation latency. it is the starting point — adjust only when the chain's workload profile or utilization demands it.
...
2. **it maximizes throughput at low utilization.** all five workloads achieve their peak stable Mgas/s at 100ms. longer block times (250ms, 500ms, 1s) reduce throughput because ev-reth idles between blocks. at 100ms with 30M gas limit: GasBurner achieves 148 Mgas/s, StatePressure 74, MixedWorkload 70, DeFi 23-33, ERC20 28.
...
the tradeoff: 100ms becomes unstable at higher utilization for small-tx workloads. when peak block fullness exceeds 20% for ERC20 or 40% for DeFi/MixedWorkload, increase block time to 250ms or 500ms. see [configuration knobs](#configuration-knobs) for the full picture.
...
|------|----------|----------|--------------|-----------|
| TestGasBurner | `30m_80pct` | 8 | pb_max < 500ms, Mgas/s > 100, non-empty > 90% | GasBurner is stable at 100ms across all utilization — 80% exercises block building throughput at the high end |
| TestERC20Throughput | `30m_10pct` | 4 | pb_max < 500ms, Mgas/s > 20, TPS > 500 | ERC20 is only stable at 100ms up to ~10% util — this is the highest stable config and tests peak TPS (703 observed) |
| TestDeFiSimulation | `30m_20pct` | 3 | pb_max < 500ms, Mgas/s > 15, TPS > 200 | DeFi is stable at 100ms up to ~20% util — this tests the DeFi stability boundary with realistic Uniswap swap load |
| TestStatePressure | `30m_40pct` | 6 | pb_max < 500ms, Mgas/s > 100, non-empty > 90% | StatePressure is stable at all 30M configs — 40% exercises storage-write-heavy execution at moderate load |
...
|-----------|----------|---------------------|-------------|
| 100ms | 10 | ~100ms | fastest confirmation. pb_avg is 30-75ms depending on workload, leaving 25-70ms headroom. stable for all workloads at low utilization. as blocks get fuller, ev-reth needs more time for EVM execution and state root computation. small-tx workloads (ERC20, DeFi) hit contention first because they require many txs per block |
| 250ms | 4 | ~250ms | eliminates stalls for MixedWorkload and DeFi through 80% target utilization. ev-reth has 2.5x more time per block to build and validate. ERC20 remains marginal (1 stall observed) |
| 500ms | 2 | ~500ms | first interval where ERC20 is fully stable. all workloads stable. blocks are larger and better-packed. the system idles between blocks, so Mgas/s decreases compared to 100ms despite blocks containing more gas each |
| 1s | 1 | ~1s | all workloads stable at all utilization levels. highest per-block gas, lowest Mgas/s. 75-99% non-empty blocks |

**when to increase block time:** when peak block fullness exceeds the stability threshold for your workload:

| workload type | stable at 100ms up to | increase to 250ms at | increase to 500ms at |
|--------------|----------------------|---------------------|---------------------|
...
too frequent (e.g., 10ms scrape at 100ms blocks) adds CPU overhead during block production. too infrequent (e.g., 200ms scrape at 100ms blocks) means metrics miss blocks entirely. the 1:4 to 1:5 range gives 4-5 scrape opportunities per block interval. this ratio is derived from configs that produced stable results — it has not been experimentally validated with mismatched values.
...
- **most resilient to fast block times.** every config at ≤100M gas limit with 100ms blocks is stable. only extreme gas limits (300M-1G) that push pb_avg well beyond block time caused issues.
...
- **block time has minimal impact on stability.** 100ms is stable for all reasonable gas limits. longer block times reduce Mgas/s without improving stability.
...
- **narrowest stable window at 100ms.** only `30m_10pct` (4 spammers) is stable at 100ms. the cliff is between 10% and 20% target utilization — `30m_20pct` (6 spammers) already shows issues.
...
- **block time sweep confirms pattern.** `30m_80pct` is unstable at 100ms, stable at 250ms and 1s.
...
- **most resilient workload overall.** only 1 of 13 configs had issues. all 30M configs at 100ms are stable, even at 120% target utilization. 1-2M gas/tx means very few txs per block, minimizing contention.
- **ev-reth achieves its highest efficiency** with storage writes: 1.143 Ggas/s stable (100m_20pct). 3.5x higher than DeFi/ERC20.
...
- **stability tracks DeFi, not ERC20.** stable at 100ms through 40% target utilization. the stall cliff is between 40% and 80%. ERC20 alone is unstable at >10%, but the GasBurner/StatePressure components (30% of spammers) fill blocks with large txs efficiently, compensating.
...
- **250ms eliminates instability.** `30m_80pct_250ms` (88.6 Mgas/s, 164 TPS, stable) vs `30m_80pct` at 100ms (116.1 Mgas/s, 222 TPS, unstable). validates profile 2 recommendation.
- **100M gas limit shows a sharp cliff.** `100m_10pct` is stable at 245.2 Mgas/s. `100m_20pct` and above are all unstable.
